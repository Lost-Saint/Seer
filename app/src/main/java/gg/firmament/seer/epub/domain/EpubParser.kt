...
import gg.firmament.seer.epub.domain.models.EpubBook
import gg.firmament.seer.epub.domain.models.EpubChapter
import gg.firmament.seer.epub.domain.models.EpubImage
import java.io.File
...
class EpubParser {
  ...
  suspend fun createEpubBook(filePath: String, shouldUseToc: Boolean = true): EpubBook {
    ...
    inputStream.use {
      return parseAndCreateEbook(it, shouldUseToc)
    }
  }
  ...
  private suspend fun parseAndCreateEbook(
      ...
      withContext(Dispatchers.IO) {
        ...
        val tocFileItem =
            manifestItems.values.firstOrNull { it.absPath.endsWith(".ncx", ignoreCase = true) }

        // Find the nested navPoints in the table of contents (toc.ncx) file.
        val tocNavPoints =
            tocFileItem?.let { navItem ->
        ...
        val chapters =
            if (!tocNavPoints.isNullOrEmpty() && shouldUseToc) {
  ...
  private suspend fun getZipFiles(inputStream: InputStream): Map<String, EpubFile> =
      withContext(Dispatchers.IO) {
  ...
  private fun createEpubDocument(files: Map<String, EpubFile>): EpubDocument {
    ...
    val opfFilePath =
        parseXMLFile(container.data)
            ?.selectFirstTag("rootfile")
            ?.getAttributeValue("full-path")
            ?.decodedURL ?: throw EpubParserException("Invalid container.xml file")

    val opfFile = files[opfFilePath] ?: throw EpubParserException(".opf file missing")

    val document =
        parseXMLFile(opfFile.data) ?: throw EpubParserException(".opf file failed to parse data")
    val metadata =
        document.selectFirstTag("metadata")
            ?: throw EpubParserException(".opf file metadata section missing")
    val manifest =
        document.selectFirstTag("manifest")
            ?: throw EpubParserException(".opf file manifest section missing")
    val spine =
        document.selectFirstTag("spine")
  ...
  private fun parseUsingSpine(
    ...
    return spine
        ...
        .filter { item ->
          chapterExtensions.any { item.absPath.endsWith(it, ignoreCase = true) } ||
  ...
  private fun parseImages(
    ...
    val unlistedImages =
        files
            ...
            .map { (_, file) -> EpubImage(absPath = file.absPath, image = file.data) }
